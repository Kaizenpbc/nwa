...
// ── In-Memory Data (reset per invocation — serverless) ──────────
...
function createServer() {
  ...
  server.tool("list_complaints", "List all complaints with optional filters. Returns summary of each complaint including SLA status.", {
...
export default async function handler(req, res) {
  if (req.method === "GET" || req.method === "DELETE") {
    res.status(405).json({
      jsonrpc: "2.0",
      error: { code: -32000, message: "Method not allowed. Stateless mode — use POST only." },
      id: null,
    });
    return;
  }
