...
// ═══════════════════════════════════════════════════════════════════
//  NWA Jamaica — Complaint Dashboard MCP Server
// ═══════════════════════════════════════════════════════════════════
...
const STATUSES = ["pending", "under_review", "sent_review", "resolved"];
...
function formatComplaint(c) {
  ...
  const daysRemaining = getSlaDaysRemaining(c);
  return {
    id: c.id,
    category: c.category,
    description: c.desc,
    ...
    sla: {
      status: sla,
      daysRemaining,
      deadlineDays: SLA_DAYS[c.priority] || 3,
    },
  };
}
...
server.tool(
  "list_complaints",
  "List all complaints with optional filters. Returns summary of each complaint including SLA status. Use this to browse or search the complaint queue.",
  {
    ...
    parish: z.string().optional().describe("Filter by parish name (e.g. 'Kingston', 'St. Andrew')"),
    category: z.string().optional().describe("Filter by category (Pothole, Road Damage, Flooding, Drainage, Signage, Other)"),
    search: z.string().optional().describe("Search keyword in complaint description or ID"),
  },
  async (params) => {
    ...
    const formatted = results.map(formatComplaint);
    return {
      content: [{
        type: "text",
        text: JSON.stringify({ total: formatted.length, complaints: formatted }, null, 2),
      }],
    };
  }
);
...
server.tool(
  "get_complaint",
  "Get full details of a single complaint by its tracking ID (e.g. NWA-20260215-0001).",
  {
    id: z.string().describe("Complaint tracking ID (e.g. NWA-20260215-0001)"),
  },
  async ({ id }) => {
    const c = complaints.find(x => x.id === id);
    if (!c) {
      return { content: [{ type: "text", text: JSON.stringify({ error: `Complaint ${id} not found` }) }] };
    }
...
server.tool(
  "create_complaint",
  "Submit a new complaint. Returns the auto-generated tracking ID. Required: category, description, parish. Optional: priority, GPS coordinates.",
  {
    ...
    lat: z.number().optional().describe("Latitude of issue location"),
    lng: z.number().optional().describe("Longitude of issue location"),
  },
  async (params) => {
    if (!PARISHES.includes(params.parish)) {
      return { content: [{ type: "text", text: JSON.stringify({ error: `Invalid parish. Valid parishes: ${PARISHES.join(", ")}` }) }] };
    }
...
server.tool(
  ...
  "Update the status of a complaint. Status flow: pending → under_review → sent_review → resolved. Resolving requires resolvedBy and closedBy (2-person closeout).",
  {
    id: z.string().describe("Complaint tracking ID"),
    status: z.enum(["pending", "under_review", "sent_review", "resolved"]).describe("New status"),
    resolvedBy: z.string().optional().describe("Name of person who resolved (required for 'resolved' status)"),
    closedBy: z.string().optional().describe("Name of second person who approved closure (required for 'resolved' status)"),
  },
  async (params) => {
    const c = complaints.find(x => x.id === params.id);
    if (!c) {
      return { content: [{ type: "text", text: JSON.stringify({ error: `Complaint ${params.id} not found` }) }] };
    }

    if (params.status === "resolved") {
      if (!params.resolvedBy || !params.closedBy) {
        return {
          content: [{
            type: "text",
            text: JSON.stringify({
              error: "2-person closeout required. Provide both 'resolvedBy' and 'closedBy' names to resolve a complaint.",
            }),
          }],
        };
      }
      if (params.resolvedBy === params.closedBy) {
        return {
          content: [{
            type: "text",
            text: JSON.stringify({
              error: "2-person closeout requires two DIFFERENT people. 'resolvedBy' and 'closedBy' cannot be the same person.",
            }),
          }],
        };
      }
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          success: true,
          message: `Status updated: ${STATUS_LABELS[oldStatus]} → ${STATUS_LABELS[params.status]}`,
          complaint: formatComplaint(c),
        }, null, 2),
      }],
    };
  }
...
server.tool(
  "assign_complaint",
  "Assign a complaint to a department for action. Valid departments: Road Maintenance, Bridge & Drainage, Parish Operations, Traffic Management, Major Projects, Planning & Research.",
  {
    id: z.string().describe("Complaint tracking ID"),
  ...
  async (params) => {
    ...
    if (!DEPARTMENTS.includes(params.department)) {
      return {
        content: [{
          type: "text",
          text: JSON.stringify({ error: `Invalid department. Valid: ${DEPARTMENTS.join(", ")}` }),
        }],
      };
    }
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          success: true,
          message: old
            ? `Reassigned from "${old}" to "${params.department}"`
            : `Assigned to "${params.department}"`,
          complaint: formatComplaint(c),
        }, null, 2),
      }],
    };
...
server.tool(
  "add_complaint_note",
  "Add an internal case note to a complaint. Notes are timestamped and attributed to the author.",
  {
    id: z.string().describe("Complaint tracking ID"),
...
server.tool(
  "get_dashboard_stats",
  "Get executive KPI dashboard statistics: total complaints, open/resolved counts, SLA metrics, priority breakdown, parish hotspots, category breakdown.",
  {},
  async () => {
    ...
    for (const c of complaints) {
      ...
      if (sla === "met") { resolved++; slaMet++; }
      if (sla === "missed") { resolved++; slaMissed++; }
    }

    // Top parishes by open complaints
    const parishHotspots = Object.entries(byParish)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 5)
      .map(([parish, count]) => ({ parish, openComplaints: count }));
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          summary: {
            total,
            open: total - (byStatus.resolved || 0),
            resolved: byStatus.resolved || 0,
            pending: byStatus.pending || 0,
            underReview: byStatus.under_review || 0,
            sentForReview: byStatus.sent_review || 0,
          },
          sla: {
            breached: slaBreached.length,
            breachedIds: slaBreached,
            atRisk: slaAtRisk.length,
            atRiskIds: slaAtRisk,
            metCount: slaMet,
            missedCount: slaMissed,
          },
          byPriority,
          byCategory,
          parishHotspots,
...
server.tool(
  "get_parish_hotspots",
  "Get complaint hotspot analysis by parish. Shows open complaint counts, top categories, and priority breakdown for each parish with active complaints.",
  {},
  async () => {
    const parishes = {};
    ...
    for (const c of complaints) {
      ...
      if (!parishes[c.parish]) {
        parishes[c.parish] = { total: 0, high: 0, standard: 0, low: 0, categories: {} };
      }
      const p = parishes[c.parish];
      p.total++;
      p[c.priority] = (p[c.priority] || 0) + 1;
      p.categories[c.category] = (p.categories[c.category] || 0) + 1;
    }

    const hotspots = Object.entries(parishes)
      .sort((a, b) => b[1].total - a[1].total)
      .map(([parish, data]) => ({
        parish,
        openComplaints: data.total,
        priorities: { high: data.high, standard: data.standard, low: data.low },
        topCategories: Object.entries(data.categories).sort((a, b) => b[1] - a[1]).map(([cat, count]) => ({ category: cat, count })),
      }));
...
server.tool(
  "escalate_complaint",
  "Escalate a complaint to high priority. Use when an issue is urgent or has public safety implications.",
  {
    id: z.string().describe("Complaint tracking ID"),
    reason: z.string().describe("Reason for escalation"),
    escalatedBy: z.string().describe("Name of person escalating"),
  },
  async (params) => {
    const c = complaints.find(x => x.id === params.id);
    if (!c) {
    ...
    if (!c.notes) c.notes = [];
    c.notes.push({
      author: params.escalatedBy,
      text: `ESCALATED to high priority. Reason: ${params.reason}`,
      timestamp: new Date().toISOString(),
    });
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          success: true,
          message: `Complaint escalated from ${oldPriority} → high priority.`,
          complaint: formatComplaint(c),
        }, null, 2),
      }],
    };
...
server.tool(
  "search_complaints",
  "Full-text search across all complaint fields — ID, description, parish, category, assigned department, notes. Returns matching complaints ranked by relevance.",
  {
    ...
    const scored = complaints.map(c => {
      ...
      if (c.notes) {
        for (const n of c.notes) {
          if (n.text.toLowerCase().includes(q)) score += 1;
        }
      }
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          query,
          total: scored.length,
          results: scored.map(x => ({ ...formatComplaint(x.complaint), relevanceScore: x.score })),
        }, null, 2),
      }],
    };
  }
...
async function main() {
  ...
  console.error("NWA Complaint Dashboard MCP server running on stdio");
}
