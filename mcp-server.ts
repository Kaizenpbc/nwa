#!/usr/bin/env npx tsx
/**
 * NWA Dashboard MCP Server
 *
 * Exposes the NWA dashboard data (complaints, projects, closures,
 * news, emergency events) as queryable MCP tools so that Claude
 * (or any MCP client) can search, filter, and summarise the data.
 */
  ...
  async ({ search, status, priority, parish, category, sla_status, limit }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: JSON.stringify({ count: results.length, complaints: enriched }, null, 2),
        },
      ],
    };
  },
  ...
  async ({ status, severity, parish }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: JSON.stringify({ count: results.length, summary, events: results }, null, 2),
        },
      ],
    };
  },
